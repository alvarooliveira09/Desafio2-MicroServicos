...
export class MailService {
  ...
  async sendMail(content: DataMessage, type: MailType) {
    console.log(
      `sendMail>> DateTime: ${new Date().toLocaleString()} User: ${content.idUser} Number: ${content.orderNumber} Value: ${content.orderValue} PaymentConfirmed: ${content.paymentConfirmed} Type: ${type} `,
    );
  }
